...
export class MjmlCarouselImage extends BodyComponent<{ index: number; }> {
  ...
  render() {
    ...
    const image = <img
      {...this.htmlAttributes({
        ...
        alt,
        style: 'images.img',
        width: parseInt(this.props.containerWidth, 10).toString(),
        border: '0',
      }, false)} />;
    ...
    return (
      ...
      >
        {href
          ? <a {...this.htmlAttributes({ href, rel, target: "_blank" }, false)}>{image}</a>
          : image
        }
      </div>
    );
