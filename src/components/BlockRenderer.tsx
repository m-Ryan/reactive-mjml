...
>(
  ...
  const nonRawSiblings =
    props.parent?.children?.filter(child => child.tagName !== 'mj-raw').length || 0;
  ...
  return useMemo(
    () => (
      <Com {...props} data={memoData} nonRawSiblings={nonRawSiblings} context={context} />
    ),
    [props, memoData, nonRawSiblings],
  );
};
